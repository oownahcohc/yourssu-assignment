...
class AuthServiceTest: BehaviorSpec({
    ...
    val authService: AuthService = spyk(AuthService(userService, userRepository, bCryptUtils))
    ...
    Given("처음 회원가입하는 경우") {
        ...
        When("정상적인 이메일, 비밀번호를 입력하면") {
            ...
            val request = SignupRequest("abcd1234@naver.com", "password", "유어슈")
            val encoded = bCryptUtils.encodePassword(request.password)
            val createDto = request.toCreateUserDto(encoded)
            val new = User.newInstance(createDto.username, createDto.email, createDto.password)
    ...
    Given("회원가입 이후 유효한 회원을 판별하는 경우") {
        ...
        val user = User.newInstance(username, email, encodePassword)
